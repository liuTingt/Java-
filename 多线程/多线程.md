...
**偏向锁**
	sync (object)  第一个线程访问的时候会记录这个线程的ID，在object的头上markword记录这个线程的ID，没有加锁。这个线程再次访问的时候会比较记录的线程ID，相同直接访问，效率很高。这种情况使用一个线程访问。
**自旋锁**
...
**3）不要以字符串常量作为锁定对象**
		...
		这种情况还会发生比较诡异的现场，比如你用到了一个类库，在该类库中代码锁定了字符串“Hello”，但是你读不到源码，所以你在自己的代码中也锁定了“Hello”，这时候就可能发生	
...
​	在语言级别进行指令中排序（CPU级别无法禁止指令重排序）
​	DCL单例：Double Check Lock（DCL：双重检查锁）

常见面试题：单例的双重检查需要给对象加volatile吗？
	需要加，不然指令重排序会出现问题。因为new一个对象的时候，编译器编译之后，指令分为三步：1给对象申请内存，初始化默认值；2赋予对象变量实际的值；3将内存内容赋值给对栈内存中的lazySingle4变量。如果不加volatile，则会出现指令重排序,重排序为1->3->2，则会存在，还没有赋予实际值的时候，lazySingle4变量指向了内存地址，第二个线程进来发现对象有值会直接使用，但是对象的值并不正确，导致数据不一致问题。
...
volatile标记，解决编译器层面的可见性和重排序问题问题。内存屏障解决了硬件层面的可见性与重排序问题。

## 7、CAS

​	Compare And Set/Swap，号称无锁优化，自旋。
​	Java中提供了一些内部带有锁的类（锁的实现是CAS），即AtomXXX类，该类本身的方法是原子性的。
