...
3. 出现并发的可能性
   ...
   还是第二点的场景，虽然避免了误删key的情况，但是同一时间有多个线程访问代码块，仍然是不完美的。
...
1. 最左匹配原则，MySQL会一直向右匹配知道遇到范围查询(>,<,between,like)，比如a=1 and b=1 and c<1 and d=1,如果建立索引（a,b,c,d）顺序的索引，d是用不到索引的，如果建立（a,b,d,c）的索引则都可以用到，abd的顺序可以任意调整。
